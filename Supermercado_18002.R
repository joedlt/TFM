...
png("Supermercado Cod Postal 18002.png",width = 1024, height = 880)
...
png("Supermercado Cod Postal 18002 metodo del codo.png" ,bg="white")
...
#### Metodo para determinar el numero de clusters mediante la libreria NbClust utiliza 30 tipos de 
...
dev.off()
png("Supermercado Cod Postal 18002 NbClust.png ")
...
## ya que los valores que el metodo del codo( Elbow Method) no es concluyente, el analisis de NbClust es ##
## 3 pero revisando el scatter plot de la segmentación los centroides siguen estando muy dispersos
...
dev.off()
png(paste("Supermercado Cod Postal 18002 Clustering Kmeans para ",N_Cluster," CLUSTERS del Modelo RFM 12M.png",sep=""),width = 1024, height = 880)
par(mfrow=c(2, 2),oma = c(1, 0, 3, 0))
...
Des_seg<-c("PERDIDOS","MAYOR COMPRA MEDIA","CLIENTE FRECUENTE","CLIENTES FRECUENCIA BAJA","CLIENTE ANTIGUO","RECIENTES")
...
write.table(RFM_comercio,"~/Master Data Science/TFM/Segmentacion/Ficheros/Segmentos_Supermercado_18002.txt",quote = TRUE,sep = ";",col.names = FALSE, row.names = FALSE) 

table(RFM_comercio$Segmento_1,RFM_comercio$Segmento_2)

View(RFM_comercio)

## al final se integran los datos en windows con el comando##
##for %f in (*.txt) do type "%f" >> Segmentacion_comercios.txt.##