...
png("Restaurante Cod Postal 07001.png",width = 1024, height = 880)
...
png("Restaurante Cod Postal 07001 metodo del codo.png",bg="white")
...
dev.off()
png("Restaurante Cod Postal 07001 NbClust.png ")
...
## se realiza el cluster con 3 y 8 tal como nos da el modelo NbClust y el elbow Method y al final se##
## estima con k=8
N_Cluster<-8
set.seed(1234)
...
png(paste("Restaurante Cod Postal 07001 Clustering Kmeans para ",N_Cluster," CLUSTERS del Modelo RFM 12M.png",sep=""),width = 1024, height = 880)
par(mfrow=c(2, 2),oma = c(1, 0, 3, 0))
...
RFM_comer_2$Dst_Cl_7<-((RFM_comer_2$RECENCIA-Segmentos$RECENCIA[7])/N_Std[1])^2+((RFM_comer_2$FRECUENCIA-Segmentos$FRECUENCIA[7])/N_Std[2])^2+((RFM_comer_2$MONETIZACION-Segmentos$MONETIZACION[7])/N_Std[3])^2
RFM_comer_2$Dst_Cl_8<-((RFM_comer_2$RECENCIA-Segmentos$RECENCIA[8])/N_Std[1])^2+((RFM_comer_2$FRECUENCIA-Segmentos$FRECUENCIA[8])/N_Std[2])^2+((RFM_comer_2$MONETIZACION-Segmentos$MONETIZACION[8])/N_Std[3])^2
...
Seg_num<-c(0,1,2,3,4,5,6,7,8)
Des_seg<-c("PERDIDOS","CLIENTE HISTORICO","RECIENTES","COMPRA MEDIA MAS BAJA","CLIENTES MAS COMPRAS","CLIENTE ESPORADICO","MAYOR COMPRA MEDIA","CLIENTE ANTIGUO","POSIBLE ABANDONO")
...
write.table(RFM_comercio,"~/Master Data Science/TFM/Segmentacion/Ficheros/Segmentos_Restaurante_07001.txt",quote = TRUE,sep = ";",col.names = FALSE, row.names = FALSE) 
